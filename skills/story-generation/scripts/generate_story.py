...
class StoryGenerator:
    ...
    def _generate_story_content(
        ...
        if not template_path.exists():
            template_path = StoryGenerator.TEMPLATE_DIR / f'{metadata.framework}-basic.template'
        ...
        variant_stories = StoryGenerator._generate_variant_stories(
            metadata, variants
        )
        ...
        interaction_test = StoryGenerator.INTERACTION_TESTS.get(
            ...
            "// Component-specific interaction test"
        )

        # Generate a11y rules
        a11y_rules = ',\n          '.join(
            StoryGenerator.A11Y_RULES.get(metadata.component_type, [
                "{ id: 'color-contrast', enabled: true }"
            ])
        )
        ...
        a11y_test = StoryGenerator.A11Y_TEST_CODE.get(
            metadata.component_type,
            "// Component-specific accessibility test"
        )
        ...
        story_content = story_content.replace('{{INTERACTION_TEST_CODE}}', interaction_test.strip())
    ...
    def _infer_control_type(prop: PropDefinition) -> Optional[str]:
        """Infer Storybook control type from prop type"""
        ...
        if '|' in prop.type and "'" in prop.type:
            ...
            if matches:
                ...
                return f"{{ control: 'select', options: [{options_str}] }}"

        # Function (action)
        if 'function' in prop_type or '=>' in prop_type or 'onclick' in prop.name.lower():
...
def main():
    ...
    parser.add_argument(
        '--output', '-o',
        help='Output file path (default: ComponentName.stories.tsx)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    if args.output:
        ...
        output_path = component_path.parent / f'{component_path.stem}.stories{component_path.suffix}'
