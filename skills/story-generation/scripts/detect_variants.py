...
class VariantDetector:
    ...
    # Boolean state props
    STATE_PROPS = ['disabled', 'loading', 'active', 'selected', 'checked', 'error', 'readonly']
    ...
    def detect_variants(component_metadata: Dict[str, Any]) -> List[Variant]:
        ...
        variants.extend(VariantDetector._detect_type_specific_variants(component_type, props))
        ...
        for prop in props:
            ...
            if prop_name.lower() in ['size', 'fontsize']:
                ...
                if union_values:
                    ...
                    size_order = ['xs', 'small', 'sm', 'medium', 'md', 'large', 'lg', 'xl']

                    for value in union_values:
                        if value.lower() in ['small', 'sm', 'xs']:
                            variants.append(Variant(
                                name='Small',
                                description=f'Small size variant',
                                args={prop_name: value},
                                priority=2
                            ))
                            ...
                            variants.append(Variant(
                                name='Large',
                                description=f'Large size variant',
                                args={prop_name: value},
                                priority=2
                            ))
    ...
    @staticmethod
    def _detect_type_specific_variants(component_type: str, props: List[Dict[str, Any]]) -> List[Variant]:
        """Detect variants specific to component type"""
        variants = []

        if component_type == 'button':
            # Check for icon prop
            has_icon = any(p['name'].lower() in ['icon', 'lefticon', 'righticon'] for p in props)
            if has_icon:
                variants.append(Variant(
                    name='WithIcon',
            ...
            # Error state
            has_error = any(p['name'].lower() in ['error', 'iserror', 'haserror'] for p in props)
            if has_error:
                variants.append(Variant(
                    name='WithError',
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Detect component variants from props'
    )
    parser.add_argument(
        'file_path',
