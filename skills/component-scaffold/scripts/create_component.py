...
COMPONENT_TYPE_DEFAULTS = {
    'button': {
        ...
        'props': [
            PropDefinition('variant', "'primary' | 'secondary' | 'outline' | 'ghost'", False, "'primary'", 'Visual style variant'),
            PropDefinition('size', "'small' | 'medium' | 'large'", False, "'medium'", 'Button size'),
    ...
    'input': {
        ...
        'props': [
            ...
            PropDefinition('type', "'text' | 'email' | 'password' | 'number'", False, "'text'", 'Input type'),
            ...
            PropDefinition('disabled', 'boolean', False, 'false', 'Disabled state'),
            PropDefinition('onChange', '(value: string) => void', False, None, 'Change handler'),
        ],
        'has_children': False,
    },
    'card': {
        ...
        'props': [
            PropDefinition('variant', "'elevated' | 'outlined' | 'flat'", False, "'elevated'", 'Card style variant'),
            PropDefinition('image', 'string', False, None, 'Card image URL'),
            PropDefinition('imageAlt', 'string', False, None, 'Image alt text'),
            PropDefinition('header', 'React.ReactNode', False, None, 'Card header content'),
            PropDefinition('footer', 'React.ReactNode', False, None, 'Card footer content'),
            PropDefinition('onClick', '() => void', False, None, 'Click handler'),
        ],
        'has_children': True,
    },
    'modal': {
        ...
        'props': [
            ...
            PropDefinition('size', "'small' | 'medium' | 'large' | 'fullscreen'", False, "'medium'", 'Modal size'),
            PropDefinition('closeOnBackdropClick', 'boolean', False, 'true', 'Close on backdrop click'),
    ...
    'table': {
        ...
        'props': [
            ...
            PropDefinition('columns', 'TableColumn<T>[]', True, None, 'Column definitions'),
            PropDefinition('sortable', 'boolean', False, 'false', 'Enable sorting'),
            PropDefinition('selectable', 'boolean', False, 'false', 'Enable row selection'),
            PropDefinition('pagination', 'boolean', False, 'false', 'Enable pagination'),
            PropDefinition('pageSize', 'number', False, '10', 'Rows per page'),
            PropDefinition('loading', 'boolean', False, 'false', 'Loading state'),
            PropDefinition('emptyMessage', 'string', False, "'No data'", 'Empty state message'),
            PropDefinition('onRowClick', '(row: T) => void', False, None, 'Row click handler'),
        ],
        'has_children': False,
    },
    'custom': {
...
class ComponentGenerator:
    ...
    def parse_custom_props(props_str: str) -> List[PropDefinition]:
        ...
        for prop_def in props_str.split(','):
            ...
            if ':' not in prop_def:
                print(f"Warning: Invalid prop format '{prop_def}', expected 'name:type'", file=sys.stderr)
                continue
    ...
    @staticmethod
    def generate_component(spec: ComponentSpec, output_path: Optional[str] = None) -> str:
        ...
        if spec.framework == 'react':
            component_content = ComponentGenerator.generate_component_content_react(spec)
...
def main():
    ...
    parser.add_argument('--typescript', action='store_true', default=True, help='Generate TypeScript')
    parser.add_argument('--props', help='Custom props (comma-separated: name:type,name2:type2)')
    parser.add_argument('--output', help='Output file path')
    parser.add_argument('--dry-run', action='store_true', help='Print to stdout instead of file')
    parser.add_argument('--json', action='store_true', help='Output component spec as JSON')
    ...
    try:
        ...
        if not args.name[0].isupper():
            print("❌ Component name must start with uppercase letter (PascalCase)", file=sys.stderr)
            sys.exit(1)
